...
function avg (mixedArr) {
  ...
  return Math.round(sumMixedArray/mixedArr.length*100)/100;
}
...
function greatestProduct(matrix) {
  ...
  for(i=0; i < matrix.length-4; i++) {
    for(j=0; j < matrix[i].length-4; j++) {
      greatProdX = matrix[i][j] * matrix[i][j+1] * matrix[i][j+2] * matrix[i][j+3];
    }
    if(greatProdX > greatProduct) {
      greatProduct = greatProdX;
    }
    greatProdY = matrix[i][j]*matrix[i+1][j]*matrix[i+2][j]*matrix[i+3][j];
  }
  if(greatProdY > greatProduct) {
    greatProduct = greatProdY;
  }
  return greatProduct;
}